    ...
    def process_folder_gui(self):
        ...
        if not directory or not output_dir:
            ...
            for video_file in video_files:
                ...
                job = Job(process_video, (video_file, MODEL_FILE, output_dir), description)
                job_queue.put(job)
...
class ProcessFileFrame(ttk.Frame):
    ...
    def process_file_gui(self):
        ...
        if not file or not output_dir:
            ...
            job = Job(process_video, (file, MODEL_FILE, output_dir), description)
            job_queue.put(job)
            self.master.master.update_queue()
...
class DownloadAndProcessFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Button(self, text='Download & Process', command=self.download_and_process_gui).grid(row=2, column=0)
    ...
    def download_and_process_gui(self):
        ...
        if not url or not output_dir:
            ...
            description = f"{url}"
            job = Job(download_and_process, (url, MODEL_FILE, output_dir), description)
            job_queue.put(job)
            self.master.master.update_queue()
